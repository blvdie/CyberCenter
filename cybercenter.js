(function () {
    ...
    if ('IntersectionObserver' in window) {
        ...
        // Страховка: если observer не сработал — показываем всё через секунду
        setTimeout(function () {
    ...
    (function () {
        ...
        function calculateDimensions() {
            var firstCard = ticker.querySelector('.review-card');
            if (firstCard) {
            ...
            if (isMobile) {
                ...
                centerOffset = (wrapWidth / 2) - (cardWidth / 2);
            }
        }
        calculateDimensions();
        ...
        function setPosition(animate) {
            ...
            var isMobile = window.innerWidth <= 760;
            var offset = isMobile ? 0 : centerOffset;
    ...
    /* Параллакс для декоративных hex-элементов */
            ...
            .then(function (data) {
                if (data.status === 'success') {
                    ...
                    successText.textContent = name.value.trim() +
                        ', ваша заявка принята! Ждём вас ' + d +
                        ' в ' + time.value +
                        ' в зоне ' + zone.options[zone.selectedIndex].text +
                        '. Администратор позвонит вам для подтверждения.';
                    form.hidden = true;
...
/* Прогресс-бар скролла, анимации и параллакс водяных знаков */
(function () {
    'use strict';
...
/* Модальные окна и cookie-баннер */
(function () {
